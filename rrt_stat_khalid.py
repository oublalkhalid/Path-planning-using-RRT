...
def run(data_dic):
    ...
    x_goal=(49, 24)  # Goal node
    environment = env.Env()
    for i in tqdm(data_dic["step_len"]):
    
        t_end_list=[]
        nb_iter_list=[]
        path_list=[]
        
        for j in tqdm(range(1)): # avoid stochastic character 
            t0=current_milli_time()
            rrt = RrtStar(environment, x_start, x_goal, i, 0.10,20,1000)
...
if __name__ == '__main__':
    ...
    dataFrame.to_pickle("result_rrtSTAR_step_len-0p1-100.pkl")
